...
abstract class MailTokenBasedOperations[U] extends SecureSocial[U] {
  ...
  protected def executeForToken(token: String, isSignUp: Boolean,
    f: MailToken => Future[Result])(implicit request: RequestHeader): Future[Result] =
    {
      env.userService.findToken(token).flatMap {
        ...
        case _ => handleSignUpError(isSignUp)
      }
    }

  def handleSignUpError(isSignUp: Boolean)(implicit request: RequestHeader) = {
   ...
   * @return the action result
   */
  def confirmationResult(gatewayRegistration: Boolean = false)(implicit request: RequestHeader): Result = {
    gatewayRegistration match {
      case false => Redirect(env.routes.loginPageUrl)
      case true => Redirect(env.routes.confirmedSignUpUrl)
    }
  }
}
